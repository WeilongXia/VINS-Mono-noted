...
BRIEF::BRIEF(int nbits, int patch_size, Type type) : m_bit_length(nbits), m_patch_size(patch_size), m_type(type)
{
    assert(patch_size > 1);
    assert(nbits > 0);
    generateTestPoints();
...
void BRIEF::compute(const cv::Mat &image, const std::vector<cv::KeyPoint> &points, vector<bitset> &descriptors,
